...
public class NeuralCarController : MonoBehaviour
{
    [Header("Car settings")]
    ...
    void GetBrainPrediction()
    {
        // Get the input vector from the brain
        float[] inputVector = new float[18];
        for (int i = 0; i < raycastDistances.Length; i++)
        {
            inputVector[i] = raycastDistances[i];
        }
        inputVector[10] = GetVelocityMagnitude();
        inputVector[11] = transform.rotation.eulerAngles.z;
        inputVector[12] = nextCheckpointDistance;

        // Get the output vector from the brain
        float[] outputVector = brain.Predict(inputVector);

        // Apply the output vector values to the car
        steeringInput = outputVector[0];
        accelerationInput = outputVector[1];
    }
